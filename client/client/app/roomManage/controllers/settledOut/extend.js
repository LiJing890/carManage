...
app.controller('roomextend',['$document','$scope','$http','dialog', '$filter', 'roomCheckServices',function ($document, $scope, $http, dialog, $filter, roomCheckServices) {
  var vm = this;
  ...
  vm.save = function(){
    if($scope.myForm.$valid){
      ...
      roomCheckServices.extendRoom(vm.jsondata).then(function (response) {
        if(response.data.code == "200"){
          vm.printData();
  ...
  vm.printData = function(){
    ...
    LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
    LODOP.ADD_PRINT_TEXT(154,6,80,25,"续房押金：");
    LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
    LODOP.ADD_PRINT_TEXT(154,79,180,25,$scope.xfdeposits + " 元");
    LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
